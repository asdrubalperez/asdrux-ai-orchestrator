...
* **Resuelto (H14, FEATURE-005 → FEATURE-006)**: los permisos híbridos de QA no confinaban realmente qué comando de Bash se ejecutaba. Resuelto en FEATURE-006: QA ya no tiene Bash en absoluto (el `TestExecutor` del propio Orquestador ejecuta el comando de test, en un contenedor Docker sin red); Developer corre confinado dentro de un contenedor Docker endurecido (sin heredar el entorno del Orquestador, sin socket de Docker del host, usuario no root). Limitación conocida y no bloqueante: el egress de red de Developer no tiene un allowlist fino (solo la red bridge default de Docker) — candidato a Feature futura si se decide necesario. Ver `docs/features/FEATURE-006-implementation-results.md`.
* Ver `docs/ROADMAP.md` para el roadmap consolidado de pendientes de diseño.
