...
- **Owner**: Asdru
- **Status**: **Approved** — Go confirmado por el owner. Spike ejecutado en la VPS y documentado en `FEATURE-007-spike-results.md`.
...
2. Existe una forma de autenticar ese mecanismo **sin login interactivo**, con criterio
   ...
   (equivalente a H4 de FEATURE-001, con este criterio de "headless real" precisado).
...
5. El proceso Codex, invocado como child process del proceso padre del Orquestador, no hereda
   `process.env` completo de ese proceso padre — verificado con una variable canario (ver Sección
   8), no solo con inspección visual del entorno heredado.
...
- Verificación de variables de entorno heredadas del proceso padre del Orquestador hacia el
  proceso Codex, con variable canario.
...
- Un wrapper de shell/sistema operativo que intercepte la ejecución de comandos de Codex — esa
  línea de solución ya fue investigada y descartada para H14 (`docs/research/H14-command-
  confinement.md`, sección 4.1) por ser frágil y específica de proveedor. Si Codex necesita
  confinamiento de comandos en el futuro (rol `qa`), el candidato es el patrón de contenedor de
  FEATURE-006, no un wrapper de shell — a decidir en una Feature futura, no en esta.
...
- Expandir el ítem de roadmap "selección de modelo por rol" (hoy acotado a Haiku vs Sonnet dentro
  de Anthropic) para cubrir también selección de *proveedor* (Claude Code vs Codex) por rol, con
  un toggle de configuración "misma config para todos los agentes" vs "una config por agente",
  parametrizado desde la capa de UI (pantalla de Disparo, hoy `[Pendiente]` en `02-ARCHITECTURE.md`).
...
5. La verificación de variables de entorno heredadas se hace inyectando una variable canario en el
   proceso padre del Orquestador (ej. `ORCHESTRATOR_SECRET_CANARY=should_not_leak_to_codex` —
   nunca un secreto real) y confirmando empíricamente que el proceso Codex, ejecutado como child
   process de ese proceso padre, no la recibe — por ejemplo inspeccionando el entorno efectivo del
   child process en la VPS, o registrando desde el proceso padre el env allowlist exacto usado
   para el spawn. No alcanza con una allowlist declarada sin esta verificación empírica.
...
- **Riesgos técnicos**:
  ...
  - **Riesgo principal señalado por Codex**: que el spike termine validando accidentalmente el
    ...
    el Orquestador. Este riesgo es precisamente lo que la Sección 3, punto 2 (criterio observable
    de "headless sin login interactivo") busca blindar.
...
| Verificación de entorno heredado | Proceso padre del Orquestador con `ORCHESTRATOR_SECRET_CANARY=should_not_leak_to_codex` (valor señuelo, no secreto real); proceso Codex invocado como child process de ese proceso padre | Se confirma empíricamente que el proceso Codex no recibe la variable canario — vía inspección del entorno efectivo del child process en la VPS, o vía registro desde el proceso padre del env allowlist exacto usado para el spawn |
...
- **Riesgo de alcance**: que la investigación del mecanismo de invocación headless (pregunta 1)
  ...
  rápido.
- **Riesgo de ambiente**: que se produzca evidencia informal en un ambiente no autorizado (ej.
  exploración en Windows local) y se confunda con evidencia formal de cierre de la Feature.
  Mitigado por la Sección 7 y la Regla 5.6.
...
**Aprobado.** Go confirmado por el owner. Discovery cerrado (alcance: invocación única; rol:
`architect`, read-only; ambiente: VPS Linux). Revisado iterativamente por Codex antes del Go
(referencia de sección corregida, criterio de canario precisado para distinguir el proceso padre
del Orquestador del wrapper de shell ya descartado en H14). Spike ejecutado en la VPS y
documentado en `FEATURE-007-spike-results.md` — ver ese documento para los 5 Functional Goals
validados con evidencia empírica.
