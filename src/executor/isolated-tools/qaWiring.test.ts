...
test("QA policy is exactly three read-only tools with no egress", () => {
  assert.deepEqual(QA_ISOLATED_POLICY.tools, ["fs_read", "fs_search", "fs_glob"]);
...
test("QA worker process exposes only read tools and receives no credential", async (t) => {
  ...
  const worker = await startQaWorker(worktree);
  try {
    assert.deepEqual(worker.effectiveTools, ["fs_read", "fs_search", "fs_glob"]);
...
test("Claude MCP stdio inventory matches QA policy and denies resources", async () => {
  const child = spawn(process.execPath, [qaMcpBridgePath()], {
    env: { ...process.env, QA_WORKER_SOCKET: "/tmp/missing-worker.sock", QA_CHANNEL_TOKEN: "synthetic" },
